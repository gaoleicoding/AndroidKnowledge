...
public class LeftLinearSnapHelper extends LinearSnapHelper {
    ...
    private final String TAG = "LeftLinearSnapHelper";
    private OrientationHelper mVerticalHelper, mHorizontalHelper;

    public LeftLinearSnapHelper() {
    }

    @Override
    public int[] calculateDistanceToFinalSnap(@NonNull RecyclerView.LayoutManager layoutManager, @NonNull View targetView) {
        int[] out = new int[2];
        // 水平方向滚动，则计算水平方向需要滚动的距离，否则水平方向滚动距离为0
        if (layoutManager.canScrollHorizontally()) {
            out[0] = distanceToStart(targetView, getHorizontalHelper(layoutManager));
        } else {
            out[0] = 0;
        }
        // 竖直方向滚动，则计算水平方向需要滚动的距离，否则水平方向滚动距离为0
    ...
    private int distanceToStart(View targetView, OrientationHelper helper) {
        ...
        Log.d(TAG, "getStartAfterPadding: " + helper.getStartAfterPadding());
    ...
    private View getStartView(RecyclerView.LayoutManager layoutManager, OrientationHelper helper) {
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            Log.d(TAG, "getDecoratedEnd: " + helper.getDecoratedEnd(child));
            Log.d(TAG, "getDecoratedMeasurement: " + helper.getDecoratedMeasurement(child));
